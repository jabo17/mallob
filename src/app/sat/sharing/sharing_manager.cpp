...
SharingManager::SharingManager(
	...
	for (size_t i = 0; i < _solvers.size(); i++) {
		...
		_produced_cls_ofs.push_back(std::ofstream(_params.logDirectory.getValAsString() + "/" + std::to_string(Process::_rank) + "/" + "produced_cls." + std::to_string(_formula_job_index) + "." + std::to_string(i) + ".log", std::ios_base::out));
	}
